...
open class Board: UIView, BoardPageViewDelegate {
    ...
    public struct Page {
        ...
        public init(title t: String, detail d: String, imageNamed i: String) {
            title = t
            detail = d
            image = i
        }
        
        public init(_ t: String, _ d: String, _ i: String) {
            title = t
            detail = d
            image = i
        }
    ...
    //FOR DATA  ------------------------
    fileprivate var pages: [Board.Page] = []
    
    //FOR DESIGN    ------------------------
    open var buttonBottom: UIButton!
    fileprivate var container: BoardPageViewController!
...
//    open var widthForAlertView: CGFloat!
    ...
    open var delegate: BoardDelegate?
    
    
    public init (pages p: [Board.Page]) {
    ...
    open func show() {
        ...
        self.container = BoardPageViewController(pages: self.pages, alertView: self)
    ...
    fileprivate func checkIfOnboardingWasSkipped(){
        ...
        if currentStep < (self.container.pages.count - 1) && !self.container.isCompleted {
            self.delegate?.board(self, didTriggerAction: .skipped, step: currentStep, total: self.container.maxStep)
        } else {
            self.delegate?.board(self, didTriggerAction: .completed, step: currentStep, total: self.container.maxStep)
        }
    }
    
    
    //MARK: FOR CONFIGURATION    --------------------------------------
    fileprivate func configure() {//pages p: [Board.Page]) {
        ...
        self.buttonBottom.addTarget(self, action: #selector(Board.onClick), for: .touchUpInside)
    ...
    fileprivate func configureConstraints(_ superView: UIView) {
        ...
        let heightConstraintForAlertView = NSLayoutConstraint.init(item: self, attribute: .height, relatedBy: .equal, toItem: superView, attribute: .height, multiplier: percentageRatioHeight, constant: 0)
        let widthConstraintForAlertView = NSLayoutConstraint.init(item: self, attribute: .width, relatedBy: .equal, toItem: superView, attribute: .width, multiplier: percentageRatioWidth, constant: 0)
        ...
        let heightConstraintForButtonBottom = NSLayoutConstraint.init(item: self.buttonBottom, attribute: .height, relatedBy: .equal, toItem: self, attribute: .height, multiplier: 0.1, constant: 0)
        let widthConstraintForButtonBottom = NSLayoutConstraint.init(item: self.buttonBottom, attribute: .width, relatedBy: .equal, toItem: self, attribute: .width, multiplier: 1, constant: 0)
        ...
        let heightConstraintForContainer = NSLayoutConstraint.init(item: self.container.view, attribute: .height, relatedBy: .equal, toItem: UIApplication.shared.keyWindow!, attribute: .height, multiplier: percentageRatioHeight * 0.9, constant: 0)
        let widthConstraintForContainer = NSLayoutConstraint.init(item: self.container.view, attribute: .width, relatedBy: .equal, toItem: UIApplication.shared.keyWindow!, attribute: .width, multiplier: percentageRatioWidth, constant: 0)
    ...
    //MARK: BoardPageViewDelegate    --------------------------------------
    
    func nextStep(_ step: Int) {
        self.delegate?.board(self, didTriggerAction: .nextStep, step: step, total: self.container.maxStep)
    }
    ...
    fileprivate func interceptOrientationChange(){
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(Board.onOrientationChange), name: NSNotification.Name.UIDeviceOrientationDidChange, object: nil)
    }
